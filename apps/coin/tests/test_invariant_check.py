...
def test_invariant_check_fails_on_missing_payment_event():
    ...
    event = CoinEvent.objects.create(event_type=CoinEventType.MINT)
...
def test_invariant_check_fails_on_unbalanced_tx():
    ...
    event = CoinEvent.objects.create(event_type=CoinEventType.TRANSFER)
    tx_id = uuid.uuid4()
    CoinLedgerEntry.objects.create(
        event=event,
        tx_id=tx_id,
...
def test_invariant_check_fails_on_suspended_user_account():
    ...
    account.status = CoinAccountStatus.SUSPENDED
...
def test_invariant_check_allows_suspended_system_account():
    ...
    system_account.status = CoinAccountStatus.SUSPENDED
