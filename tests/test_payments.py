...
from apps.payments.models import SubscriptionStatus, Wallet
...
class PaymentsTests(TestCase):
    def test_map_stripe_status(self):
        self.assertEqual(map_stripe_status("active"), SubscriptionStatus.ACTIVE)
        self.assertEqual(map_stripe_status("past_due"), SubscriptionStatus.PAST_DUE)
        self.assertEqual(map_stripe_status("unknown"), SubscriptionStatus.INCOMPLETE)
